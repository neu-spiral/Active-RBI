classdef listNode < handle
    % LISTNODE  A class to represent a doubly-linked list node.
    % Multiple listNode objects may be linked together to create linked lists.
    ...
    methods
        function node = listNode(Data)
            % LISTNODE  Constructs a dlnode object.
            if nargin > 0
                node.Data = Data;
            end
        end
...
%         end
%         
%         function node=getNodeAfter(beginNode,n)
%             % GETNODEAFTER  Returns nth node after beginNode
%             node=beginNode;
%             for(k=1:n)
%                 if(isempty(node))
%                     node=[];
