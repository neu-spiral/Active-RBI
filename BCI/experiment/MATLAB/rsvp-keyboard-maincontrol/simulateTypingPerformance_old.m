...
function simulationResults=simulateTypingPerformance(scoreStruct,imageStructs,sessionType,RSVPKeyboardParams,presentationStruct)
...
%                 targetSymbol
            ...
            if(scoreStruct.trialRejectionProbability>0)
                acceptedTrials=rand(length(resultTargetness),1)>=scoreStruct.trialRejectionProbability;
            else
                acceptedTrials=true(length(resultTargetness),1);
            end
            if RSVPKeyboardParams.matrixSpellerMode
                results.duration=length(resultTargetness)*presentationStruct.Duration.MatrixTrial+presentationStruct.Duration.Fixation;
            else
            ...
            if RSVPKeyboardParams.matrixSpellerMode
                results.trialLabels=trialLabels(acceptedTrials);
            else
                results.trialLabels=decision.nextSequence.trials(acceptedTrials);
            end
            results.scores=zeros(length(resultTargetness),1);
            results.scores(find(resultTargetness))=scoreStruct.conditionalpdf4targetKDE.getSample(targetCount);
            results.scores(find(~resultTargetness))=scoreStruct.conditionalpdf4nontargetKDE.getSample(length(resultTargetness)-targetCount);
