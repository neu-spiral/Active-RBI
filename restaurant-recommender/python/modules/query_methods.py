...
class randomQuery(object):
    ...
    def __init__(self, space_query, len_query=4):
    ...
    def update_query(self, p, p_label=None):
        ...
        tmp = [i for i in self.space_query]
...
class MomentumQuerying(object):
    ...
    def __init__(self, space_query, len_query=4, gam=.9, lam=.9):
        self.space_query = space_query
        self.lam = lam
        self.gam = gam
        self.len_query = len_query
        self.momentum = np.zeros(len(self.space_query))
        self.prob_history = []
        self.last_query = []
    ...
    def update_query(self, p, p_label=None):
        ...
        tmp_alp = [i for i in self.space_query]
    ...
    def update_momentum(self):
        ...
        if len(self.prob_history) >= 2:
            ...
            idx_prev_query = [self.space_query.index(self.last_query[i]) for i
                              in
...
class MomentumQueryingLog(object):
    ...
    def __init__(self, space_query, len_query=4, gam=1., lam=1., dlam=.9,
                 shift=0,
...
class MomentumQueryingLog2(object):
    ...
    def update_query(self, p, p_label=None):
        ...
        entropy_term = np.log(tmp + eps)
        entropy_term[np.isnan(entropy_term)] = 0

        if numSeq > 1:
            reward = (1- self.lam ) * entropy_term + (
                    self.lam / numSeq) * self.momentum
        else:
            reward = entropy_term
...
class ReyniQuery(object):
    ...
    def __init__(self, space_query, alph=1, len_query=4):
        self.space_query = space_query
        self.alph = alph
        self.len_query = len_query
        self.last_query = []
    ...
    def update_query(self, p, p_label=None):
        ...
        if self.alph == 1:
            ...
            renyi_entropy = np.ones(len(self.space_query)) * np.log(2)
        elif self.alph == np.inf:
            renyi_entropy = -np.ones(len(self.space_query)) * np.log(
                np.max(tmp) + eps)
        else:
...
class RenyiMomentumQuery(object):
    ...
    def __init__(self, space_query, alph=1, len_query=4, gam=1., lam=1.,
                 dlam=.9,
        ...
        self.Renyimomentum = np.zeros(len(self.space_query))
        self.prob_history = []
        self.last_query = []
    ...
    def update_query(self, p, p_label):
        ...
        renyi_entropy = np.zeros(len(self.space_query))

        if any(p_label):
            if self.alph == 1:
                for q in self.space_query:
                    q_indx = self.space_query.index(q)
            ...
            if self.alph == 1:
                ...
                renyi_entropy = -np.ones(len(self.alp)) * np.log(2)
            elif self.alph == np.inf:
                renyi_entropy = np.ones(len(self.alp)) * np.log(
                    np.max(tmp) + eps)
            else:
